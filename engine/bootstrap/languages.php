...
if (! function_exists('getAvailableLanguages')) {
    function getAvailableLanguages() {
        $list_lang = array_diff(scandir(realpath(__DIR__."/../../lang")), ['..', '.']);
        $rename = collect($list_lang)->map(function($val) {
            return str_replace('.json', '', $val);
        })->values()->all();
...
if (! function_exists('getSupportLanguages')) {
    function getSupportLanguages() {
        global $supportLanguages;

        $supportLangs = $supportLanguages;
        $filter = collect($supportLangs)->filter(function($val, $key) {
            return in_array($key, getAvailableLanguages());
        })->all();
